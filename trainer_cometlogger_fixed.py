...
class Trainer:
    ...
    def before_train(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                ...
                self.comet_logger = CometLogger.initialize_comet_logger(
                    self.args,
                    self.exp,
                    self.evaluator.dataloader.dataset
                )
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            if self.rank == 0:
                ...
                if self.args.logger == "comet":
                    metrics = {"train/" + k: v.latest for k, v in loss_meter.items()}
                    metrics.update({
                        "train/lr": self.meter["lr"].latest
                    })
                    self.comet_logger.log_metrics(metrics, step=self.progress_in_iter)
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            ...
            if self.args.logger == "comet":
                ...
                # self.comet_logger.log_images(predictions)  # 必要なら定義
            logger.info("\n" + summary)
        synchronize()
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False, ap=None):
        if self.rank == 0:
            ...
            if self.args.logger == "comet":
                self.comet_logger.save_checkpoint(
                    self.file_name,
                    ckpt_name,
                    update_best_ckpt,
                    metadata={
